function forecastingML( file,obj )
...
while ~isempty(node)  
    if strcmpi(node.getNodeName, 'metric')
        ...
        while ~isempty(subNode)
            ...
            if strcmpi(subNode.getNodeName, 'predictionStep')
                predictionStep = subNode.getTextContent;
            end
        ...
        %calculate
        flag = 2;
        mat2arff(data,flag,metricName,'forecasting.arff');
    end
